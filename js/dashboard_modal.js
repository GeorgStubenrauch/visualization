...
*/
function drawDashboard_modal(in_div, in_filename, in_attributes, in_colors){
	...
	function draw(div,draw_data,attributes) {
		...
		function histoGram(fD){
			var hG={},    hGDim = {t: 40, r: 0, b: 75, l: 80};
		...
		function pieChart(pD){
			...
			var piesvg = d3.select(div).append("svg").attr('class','pie_mw')//;
		...
		function legend(lD){
			...
			var legend = d3.select(div).append("table").attr('class','legend_mw');
			...
			leg.update = function(nD){
				...
				l.select(".legendFreq_mw").text(function(d){ return d3.format(",")(d.values_y);});

				// update the percentage column:
				l.select(".legendPerc_mw").text(function(d){ return getLegend(d,nD);});        
			}
